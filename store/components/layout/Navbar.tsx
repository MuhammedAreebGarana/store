    ...
    <header className="sticky top-0 z-50 bg-paper border-b border-ink/10">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-8">
          <Link href="/products" className="text-sm text-muted hover:text-ink transition-colors tracking-wide">
            Shop
          </Link>

          {user ? (
            <button onClick={handleSignOut} className="text-sm text-muted hover:text-ink transition-colors tracking-wide">
              Sign out
            </button>
          ) : (
            <Link href="/auth/login" className="text-sm text-muted hover:text-ink transition-colors tracking-wide">
              Sign in
            </Link>
          )}
